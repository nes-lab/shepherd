...
plug_dim = [1.6 + 0.25, 54.4 + 0.30, 8 + 0.2];
plug_distance = 19.9;
plug_count = 5;
d_clamp = 20;
...
difference () {
    ...
    translate([0,0,0.99]) round_cuboid(case_x-5, case_y-12, case_z-1, 2);

    plug_range = (plug_count - 1) / 2;
    narrowing_x = (d_clamp/2 + plug_dim[0]/2 - s_clamp);
    narrowing_y = (plug_dim[1] - .1) / 2;
    clamp_xyz = [plug_distance-3.5,1.5,plug_dim[2]];
    slot_xyz = [plug_distance-2.0, case_y-8-4.0, case_z+1];
    for (i = [-plug_range:1:plug_range]) {
        ...
        translate([i*plug_distance,+plug_dim[1]/2 + 1.8,0]) cube(clamp_xyz, center=true);
        translate([i*plug_distance,-plug_dim[1]/2 - 1.8,0]) cube(clamp_xyz, center=true);
