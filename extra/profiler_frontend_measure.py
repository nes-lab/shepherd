...
logger = logging.getLogger("shp.profilerTool")
...
def meas_emulator_setpoint(rpc_client, smu, voltage_V, current_A):
    ...
    if smu is not None:
        ...
        current_A = rpc_client.convert_raw_to_value(
...
def meas_harvester_setpoint(rpc_client, smu, voltage_V, current_A):
    ...
    voltage_adc_V = rpc_client.convert_raw_to_value(
...
def measure(host, user, password, outfile, smu_ip, harvester, emulator, short):
    ...
    with Keithley2600(f"TCPIP0::{smu_ip}::INSTR") as kth, Connection(
        ...
        if short:
            ...
            voltages_V = np.append([5.0, 0.05], np.arange(0.0, 5.1, 0.4))
            currents_A = [
                0e-6,
                1e-6,
                2e-6,
            ...
            voltages_V = np.append([0.05], np.arange(0.0, 5.1, 0.2))
            currents_A = [
                0e-6,
                1e-6,
                2e-6,
        ...
        voltages_res_V = np.append([0.05], np.arange(0.0, 5.1, 0.1))

        results_a = results_b = results_h = None

        click.echo(INSTR_EMU)
        logger.info(
            f" -> Profiler will sweep through {len(voltages_V)} voltages and {len(currents_A)} currents"
        )
        usr_conf1 = mode_unattended or click.confirm(
            "Confirm that everything is set up ...", default=True
        )
        ...
        if usr_conf1 and harvester:
            ...
            for index, (voltage, current) in enumerate(
                itertools.product(voltages_V, currents_A)
            ):
                cdata, vdata, v_meas, c_set = meas_harvester_setpoint(
                    rpc_client, kth.smub, voltage, current
                )
        ...
        if usr_conf1 and emulator:
            ...
            for index, (voltage, current) in enumerate(
                ...
                cdata, v_meas, c_set = meas_emulator_setpoint(
                    rpc_client, kth.smua, voltage, current
                )
                results_a[0][index] = voltage
                results_a[1][index] = rpc_client.convert_value_to_raw(
        ...
        if (
            ...
            and (
                mode_unattended
                or click.confirm(
                    "Confirm that everything is set up for Part 2 (Port B) ...",
                    default=True,
                )
            )
        ):
            ...
            for index, voltage in enumerate(voltages_res_V):
                cdata, v_meas, c_shp = meas_emulator_setpoint(
                    rpc_client, None, voltage, None
                )
                results_b[0][index] = voltage
                results_b[1][index] = rpc_client.convert_value_to_raw(
                    "emulator", "dac_voltage_b", voltage
                )
        ...
        if usr_conf1:
            np.savez_compressed(
                file_path, emu_a=results_a, emu_b=results_b, hrv=results_h
            )
            cnx.sudo("systemctl stop shepherd-rpc", hide=True, warn=True)
        logger.info(f"Profiling took {(time.time() - time_now):.1f} s")
