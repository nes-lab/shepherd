...
int ap_init()
{
	...
	if ((rc = dp_write(DP_REG_ABORT, 0x0000001E)))
		return rc;
	if ((rc = dp_write(DP_REG_SELECT, 0x0)))
		return rc;
	if ((rc = dp_write(DP_REG_CTRLSTAT, 0x50000000)))
		return rc;

	unsigned int retries = 10;
	do {
		if ((rc = dp_read(&data, DP_REG_CTRLSTAT)))
			return rc;
		retries--;
	...
	if ((rc = ap_write(AP_REG_CSW, 0x23000052)))
		return rc;
...
int ap_exit()
{
	return dp_write(DP_REG_CTRLSTAT, 0x0);
}
