...
static uint32_t lookup_V_oc_uV_n8(const uint64_t SoC_u_n32)
{
    ...
    if (pos_soc >= VOC_LUT_SIZE) {
        pos_soc = VOC_LUT_SIZE - 1;
    }
...
static uint32_t lookup_R_series_KOhm_n32(const uint64_t SoC_u_n32)
{
    ...
    if (pos_soc >= RSERIES_LUT_SIZE) {
        pos_soc = RSERIES_LUT_SIZE - 1;
    }
...
{
    return (uint32_t) (state.V_bat_uV_n8 >> 8u);
}
...
{
    return (uint32_t) (state.SoC_u_n32 >> 32u);
}
...
{
    state.I_out_nA_n4 = I_out_nA << 4u;
}

void set_I_in_nA(const uint64_t I_in_nA)
{
    state.I_in_nA_n4 = I_in_nA << 4u;
}
...
void update_states()
{
    ...
    if (state.I_in_nA_n4 > I_out_nA_n4) {
        ...
        if (state.SoC_u_n32 > (1000000ul << 32u)) {
            state.SoC_u_n32 = 1000000ul << 32u;
        }
        ...
        if (state.SoC_u_n32 > SoC_delta_u_n32) {
            state.SoC_u_n32 -= SoC_delta_u_n32;
        } else {
            state.SoC_u_n32 = 0;
        }
