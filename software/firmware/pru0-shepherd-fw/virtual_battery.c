...
void     set_SoC_u(const uint32_t SoC_u)
{
...
void update_states()
{
    const uint64_t I_leak_nA_n4 = mul64(state.SoC_u_n32 >> 32u, cfg->Constant_1_per_kOhm_n18) >> 14u;
    ...
    if (state.I_in_nA_n4 > I_out_nA_n4)
    {
        ...
        const uint64_t SoC_delta_u_n32 = mul64(cfg->Constant_s_per_mAs_n48, I_delta_nA_n4) >> 20u;
