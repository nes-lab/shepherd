...
def get_trace_iv_address() -> int:
    with Path("/sys/shepherd/memory/iv_address").open(encoding="utf-8") as f:
        return int(f.read().rstrip())


def get_trace_iv_size() -> int:
    with Path("/sys/shepherd/memory/iv_size").open(encoding="utf-8") as f:
        return int(f.read().rstrip())
