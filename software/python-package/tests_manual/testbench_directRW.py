...
BBB
    ...
    RUN with duration 60 s, compression lzf, random False
        ...
        Size if = 51.24 MB,  fo = 51.252 MB,  fn = 21.729 MB,  fnts = 51.252 MB
    RUN with duration 60 s, compression 1, random False
        ...
        Size if = 25.203 MB,  fo = 25.205 MB,  fn = 9.859 MB,  fnts = 25.205 MB
    RUN with duration 60 s, compression None, random False
        ...
        Size if = 91.633 MB,  fo = 91.633 MB,  fn = 45.832 MB,  fnts = 91.633 MB
    RUN with duration 60 s, compression lzf, random True
        ...
        Size if = 75.344 MB,  fo = 75.356 MB,  fn = 45.832 MB,  fnts = 75.356 MB
    RUN with duration 60 s, compression 1, random True
        ...
        Size if = 59.797 MB,  fo = 59.799 MB,  fn = 44.452 MB,  fnts = 59.799 MB
...
import numpy as np
from shepherd_core import Writer, Reader, Compression
...
def generate_harvest(path: Path, duration: int, compression: Compression, random: bool = True) -> None:
    rng = np.random.default_rng()
    samples_per_1s = Writer.samples_per_buffer * 10
    with Writer(path, mode="harvester", datatype=EnergyDType.ivsample, verbose=False, force_overwrite=True, compression=compression) as sw:
        sw.store_hostname("Hrv")
        for _iter in range(duration):
            if random:
                _v = rng.uniform(low=1.0, high=3.0, size=samples_per_1s)
                _i = rng.uniform(low=.001, high=0.05, size=samples_per_1s)
            else:
...
def ram_to_file_old(path: Path, mem: SharedMemory, compression: Compression) -> None:
    with Writer(path, mode="emulator", datatype=EnergyDType.ivsample, verbose=False, force_overwrite=True, compression=compression) as sw:
...
def file_to_ram_new(path: Path, mem: SharedMemory) -> None:
    with Reader(path, verbose=False) as sr:
        shared_array = np.ndarray(shape=(mem.size//4,), dtype="u4", buffer=mem.mapped_mem)
        for _iter in range(mem.buffer_count):
            ...
            sr.ds_voltage.read_direct(shared_array, np.s_[f_start:f_end], np.s_[m_start:m_start+Writer.samples_per_buffer])
            sr.ds_current.read_direct(shared_array, np.s_[f_start:f_end], np.s_[m_start+Writer.samples_per_buffer:m_end])
...
def ram_to_file_new(path: Path, mem: SharedMemory, compression: Compression) -> None:
    with Writer(path, mode="emulator", datatype=EnergyDType.ivsample, verbose=False, force_overwrite=True, compression=compression) as sw:
        ...
        for _iter in range(mem.buffer_count):
            ...
            sw.ds_voltage.write_direct(shared_array, np.s_[m_start:m_start+Writer.samples_per_buffer], np.s_[f_start:f_end])
            sw.ds_current.write_direct(shared_array, np.s_[m_start+Writer.samples_per_buffer:m_end], np.s_[f_start:f_end])
        sw.h5file.flush()
...
def ram_to_file_new_ts(path: Path, mem: SharedMemory, compression: Compression) -> None:
    with Writer(path, mode="emulator", datatype=EnergyDType.ivsample, verbose=False, force_overwrite=True, compression=compression) as sw:
        sw.store_hostname("Emu")
        time_series_ns = sw.sample_interval_ns * np.arange(Writer.samples_per_buffer).astype("u8")
        ...
        for _iter in range(mem.buffer_count):
            ...
            if f_end > sw.ds_voltage.size:
                ...
                sw.ds_voltage.resize((f_end,))
                sw.ds_current.resize((f_end,))
            sw.ds_time[f_start:f_end] = _iter / 10 + time_series_ns  # TODO: not really needed anymore
            sw.ds_voltage.write_direct(shared_array, np.s_[m_start:m_start+Writer.samples_per_buffer], np.s_[f_start:f_end])
            sw.ds_current.write_direct(shared_array, np.s_[m_start+Writer.samples_per_buffer:m_end], np.s_[f_start:f_end])
        sw.h5file.flush()
...
if __name__ == "__main__":
    ...
    path_i = Path(__file__).parent / "artiHrv.h5"  # "/var/shepherd/recordings/artiHrv.h5")
    path_o1 = Path(__file__).parent / "artiEmu1.h5"  # "/var/shepherd/recordings/artiEmu.h5")
    path_o2 = Path(__file__).parent / "artiEmu2.h5"  # "/var/shepherd/recordings/artiEmu.h5")
    path_o3 = Path(__file__).parent / "artiEmu3.h5"  # "/var/shepherd/recordings/artiEmu.h5")
    duration = 60
    ...
    for random in [False, True]:
        for compression in compressions:
            ...
            with SharedMemory(duration) as mem:
                time.sleep(1)
                two = timeit("file_to_ram_old(path_i, mem)", globals=globals(), number=1)
                time.sleep(1)
                tro = timeit("ram_to_file_old(path_o1, mem, compression)", globals=globals(), number=1)
                # TODO: compare files - content should be identical
                #       add sleep & sync inbetween
                #       theory: its only faster because timestamp is ignored?
                time.sleep(1)
                twn = timeit("file_to_ram_new(path_i, mem)", globals=globals(), number=1)
                time.sleep(1)
                trn = timeit("ram_to_file_new(path_o2, mem, compression)", globals=globals(), number=1)
                trt = timeit("ram_to_file_new_ts(path_o3, mem, compression)", globals=globals(), number=1)
                print(f"RUN with duration {duration} s, compression {compression}, random {random}")
                print(f"    Old F2RAM = {round(two,3)} s, RAM2F = {round(tro,3)} s")
                print(f"    New F2RAM = {round(twn,3)} s, RAM2F = {round(trn,3)} s, RAM2Fts = {round(trt,3)}")
                print(f"    Size if = {round(path_i.stat().st_size/2**20,3)} MB, "
                      f" fo = {round(path_o1.stat().st_size/2**20,3)} MB, "
                      f" fn = {round(path_o2.stat().st_size/2**20,3)} MB, "
                      f" fnts = {round(path_o3.stat().st_size/2**20,3)} MB")
                time.sleep(1)
                path_i.unlink()
                path_o1.unlink()
                path_o2.unlink()
