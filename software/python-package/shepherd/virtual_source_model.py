...
class VirtualSourceModel(object):
    ...
    def __init__(self, setting, calibration: CalibrationData):
        ...
        self.V_enable_output_threshold_uV = values[
            9
        ...
        self.interval_check_thresholds_n = values[
            12
        ]  # some BQs check every 65 ms if output should be disconnected

        self.V_pwr_good_enable_threshold_uV = values[
            13
        ...
        # boost converter
        self.V_input_boost_threshold_uV = values[
            17
        ...
        self.LUT_inp_efficiency_n8 = values[
            24
    ...
    def update_states_and_output(self) -> int:
        ...
        if self.is_outputting or self.interval_startup_disabled_drain_n:
            if (not self.enable_buck) or (
                self.V_mid_uV <= self.V_output_uV + self.V_buck_drop_uV
            ):
